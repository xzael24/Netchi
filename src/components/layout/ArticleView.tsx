...
export function ArticleView({
  ...
  return (
    <main className="bg-[#1A3CDB] text-cream w-screen min-h-screen flex flex-col">
      ...
      <div ref={contentRef} className="flex-1 px-6 md:px-10 pt-[72px] pb-16 lg:pt-[64px]">
        <div className="mx-auto max-w-3xl">
          <div data-article-reveal className={`mb-8 flex items-center justify-between border-2 ${LINE} bg-cream/5 px-4 py-2`}>
            ...
            <MenuButton onClick={() => router.push(backHref)} label={t("av.back")} />
          </div>
          ...
          {(article.date || article.source || (article.pwnCount != null && article.pwnCount > 0)) && (
            ...
            {article.pwnCount != null && article.pwnCount > 0 && (
              <>
                <span className="text-cream/30">·</span>
                <span className="font-mono text-[11px] text-cream/70">{t("av.affected", { n: article.pwnCount.toLocaleString("id-ID") })}</span>
              </>
            )}
          </div>
          )}
