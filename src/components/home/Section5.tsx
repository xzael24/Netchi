...
function AccordionItem({
  ...
  return (
    <div className={`border-b-2 ${LINE}`}>
      ...
      >
        <span className="font-display font-bold text-cream leading-tight tracking-[-0.02em] text-[clamp(1rem,3cqw,1.6rem)]">
          {qa.question}
        </span>
        <span className={`shrink-0 font-mono text-cream text-[clamp(1rem,3cqw,1.5rem)] transition-transform duration-300 ${open ? "rotate-45" : ""}`}>
          +
        </span>
      </button>
      <div
      ...
      >
        <div className="overflow-hidden">
          <p className="pb-6 font-display text-cream/70 leading-relaxed text-[clamp(0.8rem,2.5cqw,1rem)]">
            {qa.answer}
          </p>
        </div>
      </div>
...
export function Section5() {
  ...
  return (
    <section className="bg-[#1A3CDB] text-cream w-screen min-w-full">
      {/* Mobile: flex-col */}
      <div className="lg:hidden flex flex-col px-5 py-8 border-b-2 border-cream/25">
        <div className="flex items-end justify-between mb-4">
          <h2 className="font-display font-bold leading-[1] tracking-[-0.03em] text-5xl">FAQ</h2>
          <span
            className="font-mono font-extrabold uppercase tracking-widest text-cream/60 whitespace-nowrap"
      ...
      <div className="hidden lg:grid w-full min-w-full">
        ...
        <div className={`grid grid-cols-[2.6%_94.05%_3.35%] grid-rows-[22vh] border-b-2 ${LINE}`}>
          ...
          <div className={`border-r-2 ${LINE} flex flex-col items-start justify-end pl-2 md:pl-4 pb-2 relative container-cell`}>
            ...
            <span className="font-mono font-extrabold uppercase tracking-widest text-cream/60 text-[clamp(0.4rem,2cqw,0.7rem)]">(5)//FAQ</span>
            <h2 className="font-display font-bold text-cream leading-[0.9] tracking-[-0.03em] text-[clamp(2.5rem,10cqw,5.5rem)]">
              FAQ
            </h2>
          </div>
        ...
        <div className={`grid grid-cols-[2.6%_94.05%_3.35%] ${LINE}`}>
          ...
          <div className={`border-r-2 ${LINE} container-cell px-4 md:px-8`}>
            {FAQS.map((qa, i) => (
              <AccordionItem key={i} qa={qa} open={openIndex === i} onToggle={() => setOpenIndex(openIndex === i ? null : i)} />
            ))}
          </div>
