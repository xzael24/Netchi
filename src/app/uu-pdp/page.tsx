...
// UU PDP kategorisasi & pasal tetap dalam Bahasa Indonesia (content edukasi),
// hanya label "Semua" filter yang diterjemahkan.
const CATEGORY_KEYS: Partial<Record<(typeof UU_CATEGORIES)[number], TranslationKey>> = {
  Semua: "uu.all",
};

const showCategoryLabel = (c: string, t: (k: TranslationKey) => string) =>
  CATEGORY_KEYS[c as (typeof UU_CATEGORIES)[number]] ? t(CATEGORY_KEYS[c as (typeof UU_CATEGORIES)[number]]!) : c;
...
export default function UuPdpPage() {
  ...
  return (
    <main className="bg-[#1A3CDB] text-cream w-screen min-h-screen flex flex-col pt-[25px] lg:pt-[35px] pb-14">
      <Navbar />
        ...
        <div className={`border-r-2 ${LINE} p-4 md:p-5`}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <input
              ...
              placeholder={t("uu.searchPlaceholder")}
              aria-label={t("uu.searchAria")}
      ...
      <div className="lg:hidden flex items-center justify-between border-b-2 border-cream/25 px-5 py-4">
        ...
        <span className="font-mono text-[10px] uppercase tracking-widest text-cream/50">{t("uu.countLabel", { n: filteredUU.length })}</span>
      </div>
