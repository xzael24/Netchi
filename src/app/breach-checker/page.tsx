...
export default function BreachCheckerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const pw = input.trim();
    if (!pw) {
      setResult({ status: "error", message: "Password tidak boleh kosong" });
      return;
    }
    ...
    if (res.error) {
      setResult({ status: "error", message: `Tidak bisa terhubung ke HIBP (${res.error}). Coba lagi.` });
      return;
    }
    ...
    <FeatureShell index="1" label="1://SCAN">
      <div className="flex flex-col items-center justify-center py-10 text-center">
        ...
        <p className="mt-4 max-w-lg text-center font-body text-cream/70 text-sm md:text-base">
          Password dicek terhadap 700+ juta password yang bocor via Have I Been Pwned.
          Password di-hash SHA-1 di perangkatmu — hanya 5 karakter prefix yang dikirim
          (k-anonymity), jadi password tidak pernah meninggalkan device.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-10 w-full max-w-xl" noValidate>
          <div className={`flex border-2 ${LINE}`}>
            ...
            >
              {result.status === "checking" ? "Mengecek…" : "Cek →"}
            </button>
          </div>
        ...
        {result.status === "common" && (
          <div className="mt-10 w-full max-w-xl border-2 border-[#ff6b6b]/50 bg-cream/5 p-5 text-left">
            <span className="font-mono text-xs uppercase tracking-widest text-[#ff6b6b]">⚠ Password terlalu umum</span>
            <p className="mt-2 text-sm text-cream/80">
              Password ini masuk daftar password paling umum di dunia — pasti sudah ada di
              database peretas. Ganti sekarang dengan password panjang & acak.
            </p>
        ...
        {result.status === "pwned" && (
          <div className="mt-10 w-full max-w-xl border-2 border-[#ff6b6b]/50 bg-cream/5 p-5 text-left">
            <div className="flex items-center justify-between">
              <span className="font-mono text-xs uppercase tracking-widest text-[#ff6b6b]">⚠ Password pernah bocor</span>
            ...
            <p className="mt-3 text-sm text-cream/80 leading-relaxed">
              Password ini ditemukan <b className="text-[#ff6b6b]">{result.count.toLocaleString("id-ID")}×</b> dalam
              kebocoran data yang diketahui. Risiko tinggi — jangan pernah pakai password ini lagi.
            </p>
            <div className={`mt-4 border-2 ${LINE} p-4`}>
              ...
              <ul className="mt-2 list-inside list-disc text-sm text-cream/80">
                <li>Ganti password yang terpakai di akun mana pun sekarang.</li>
                <li>Aktifkan 2FA lewat aplikasi authenticator.</li>
                <li>Pakai password unik & acak per akun.</li>
              </ul>
              <Link href="/password" className="mt-3 inline-block font-mono uppercase tracking-widest text-[#ff4d4d]">
                Generate password kuat →
              </Link>
        ...
        {result.status === "safe" && (
          <div className="mt-10 w-full max-w-xl border-2 border-cream/25 bg-cream/5 p-5 text-left">
            <div className="flex items-center justify-between">
              <span className="font-mono text-xs uppercase tracking-widest text-[#4cd99b]">✓ Tidak ditemukan dalam kebocoran</span>
              <span className="font-mono text-[10px] uppercase tracking-widest text-[#4cd99b]">● Data real (HIBP)</span>
            </div>
            <p className="mt-2 text-sm text-cream/80">
              Password ini tidak ditemukan dalam 700+ juta password bocor yang diketahui.
              Tetap jangan pakai satu password untuk semua akun.
            </p>
          </div>
        )}
