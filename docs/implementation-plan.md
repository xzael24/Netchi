...
## Final Status (15 Agu 2026 — semua item plan sudah selesai)

| Area | Status |
|------|--------|
| 5 halaman fitur (breach-checker, privacy-score, password, uu-pdp, dummy-data) | ✅ Selesai, dengan validasi input (`src/lib/validate.ts`) |
| API routes (src/app/api/) | ⏭️ Tidak dibuat — arsitektur 100% client-side (tanpa login, tanpa database), sesuai PRD. Tidak ada permintaan keluar selain HIBP k-anonymity |
| i18n ID/EN | ✅ Selesai — `LocaleProvider` (localStorage `netchi-locale`), `src/lib/i18n.ts` (tipesafe via `TranslationKey`), `<html lang>` dinamis |
| Security headers | ✅ Selesai + audit — CSP, HSTS (prod), nosniff, Referrer-Policy, X-Frame-Options, Permissions-Policy, poweredByHeader off. `X-XSS-Protection` di-skip (deprecated) |
| Middleware | ⏭️ Tidak diperlukan (tidak ada route yang butuh auth/proteksi) |
| Testing | ✅ Lighthouse (desktop 99-100, mobile 68-80), manual checklist multi-viewport, Playwright smoke |
| README | ✅ Di-overhaul (fitur, tech stack, struktur, keamanan, i18n, deploy) |
| Env validation | ⏭️ Tidak ada env/API key yang dipakai — HIBP pakai endpoint gratis (range, k-anonymity), tanpa key |
| `src/lib/sanitize.ts` | ⏭️ Tidak dibuat — tidak ada URL/HTML dari input user; CSV export sudah RFC 4180 escaping |

**Catatan keputusan yang menyimpang dari plan (dengan alasan):**
1. `X-XSS-Protection` tidak dipasang — header deprecated & bisa mengganggu (audit keamanan 15 Agu).
2. CSP memakai `script-src 'unsafe-inline'` (dokumentasikan di `next.config.ts`) — dibutuhkan inline hydration scripts Next; tidak ada sink XSS (0 `dangerouslySetInnerHTML`/`eval`).
3. Input breach checker adalah **password** (bukan email seperti draft awal plan) — sesuai PRD 5.1; HIBP k-anonymity tanpa API key.
